...
func TestDevCacheGateTier(t *testing.T) {
	...
	for _, c := range []string{DevCacheCategoryGo, DevCacheCategoryGoModCache, DevCacheCategoryCargo, DevCacheCategoryNuGet, DevCacheCategoryNuGetGlobalPackages, DevCacheCategoryUV, DevCacheCategoryBun, DevCacheCategoryJetBrainsIDECaches, DevCacheCategoryVisualStudioCaches} {
...
func TestPlanNeedsDistinctiveProcessDetection(t *testing.T) {
	...
	if !planNeedsDistinctiveProcessDetection(map[string]bool{DevCacheCategoryGoModCache: true}) {
		t.Fatal("go-modcache should need distinctive-process detection")
	}
