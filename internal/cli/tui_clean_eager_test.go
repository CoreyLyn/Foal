...
func TestEagerCleanModelDefaultSelectionAndCursorIndependence(t *testing.T) {
	...
	// Complete rule matrix: 1 default + 30 permanent = 31; 6 Recycle Bin opt-ins
	// unselected (the 6th is exact-selection-only nvidia_installer_cache).
	if defaults != 1 || permanentOptIns != 30 || recycleBinOptIns != 6 || wantSelected != 31 {
		t.Fatalf("matrix selection defaults=%d permanent=%d rb_opt_ins=%d wantSelected=%d; want 1/30/6/31",
	...
	if len(model.rows) != 38 {
		t.Fatalf("eager rows = %d, want 38 executable categories", len(model.rows))
	}
