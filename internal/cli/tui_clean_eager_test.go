...
func TestEagerCleanModelDefaultSelectionAndCursorIndependence(t *testing.T) {
	...
	// Complete rule matrix: 1 default + 31 permanent = 32; 6 Recycle Bin opt-ins
	// unselected (the 6th is exact-selection-only nvidia_installer_cache).
	if defaults != 1 || permanentOptIns != 31 || recycleBinOptIns != 6 || wantSelected != 32 {
		t.Fatalf("matrix selection defaults=%d permanent=%d rb_opt_ins=%d wantSelected=%d; want 1/31/6/32",
	...
	if len(model.rows) != 39 {
		t.Fatalf("eager rows = %d, want 39 executable categories", len(model.rows))
	}
