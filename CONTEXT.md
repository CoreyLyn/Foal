...
**TUI status markers**:
Presentation-only symbols used by interactive Foal views to make review and execution reliability easier to scan. The Clean TUI uses `…` or a spinner for waiting/in-progress work, `✓` for complete-with-candidates or cleaned, `–` for empty, `⊘` for a safety skip, `?` for analysis-required state, and `!` for partial, incomplete, failed, or canceled outcomes; the `>` cursor and `[x]`/`[ ]` selection remain separate. When color is available, marker hues reinforce success, attention, skipped, progress, or empty state without using pure red; `NO_COLOR` keeps the symbols and selection semantics. No marker or color implies cleanup authorization.
_Avoid_: JSON status code, byte-derived progress percentage, cleanup authorization, safe-to-delete signal, color-only state
...
A narrow presentation exception that may apply ANSI/style sequences to individual path-free tokens (byte sizes, state markers, confirmation warnings) after a plain-text frame is built, while keeping the plain frame the source of truth for tests and copy. Clean composition declares semantic line roles so headings, progress, prose, totals, and hints do not depend on matching already rendered wording; pattern matching remains a fallback for shared plain-text viewers. It does not authorize whole-row rainbow styling or embed escapes inside asserted contract text.
...
The exact per-run set of canonical default and opt-in category identifiers chosen for Clean TUI confirmation. Default categories and every category with Permanent-delete eligibility start selected but may be cleared; Recycle Bin opt-in categories and Windows servicing start unselected. The current catalog therefore starts with 37 selected categories (1 default + 36 permanent, including `user_temp`, `crash_dumps`, `explorer_thumbnail_cache`, `inet_cache`, `nvidia_gl_cache`, `vrchat_cache`, `trae_cache`, and `obsidian_cache`). The seven Recycle Bin opt-ins start unselected: `windows_error_reporting`, exact-selection-only `nvidia_installer_cache`, `lghub-cache`, `thunder-update-download`, `windows-temp`, and `windows-update-download-cache`, plus Standard-selection `electron-updater-residue`; exact-selection-only `winsxs_component_store` also starts unselected and requires its separate analysis action before it can be selected. Confirmation authorizes exactly the visible selection and must not silently add a category or consume preview paths; an `empty`, `skipped`, `incomplete`, or `failed` outcome removes and disables that category for the current scan. CLI additive Opt-in remains unchanged.
...
The single separate TUI view that reviews the exact Clean TUI cleanup selection before execution; entering it performs no cleanup, and only a second Enter authorizes the shared Clean execution path to resolve and validate fresh work. It separately reports Permanent deletion and Recycle Bin category, candidate, and byte totals plus any path-free Windows servicing operation, labels every selected category with its Planned action, states that permanent deletion is not recoverable, discloses servicing/UAC impact, and retains category-specific impact notices. It becomes available only when the selection is non-empty and every scannable category has a Clean TUI category scan outcome. One confirmation supplies the action-specific authorization for every disclosed group; fresh resolution may change candidates and bytes but cannot introduce an undisclosed action type.
...
The terminal, path-free projection of one selected category's fresh execution: `empty`, `cleaned`, `partial`, `skipped`, `failed`, or `canceled`. `partial` means at least one item succeeded alongside any excluded, skipped, failed, or canceled item; action-specific and Affected bytes count only successful operations, processed-category progress counts only terminal categories, and item-level Result and history remain authoritative. A non-successful final outcome may carry one stable Foal-owned reason code projected from its own evidence; an otherwise empty category may use a run-level issue as fallback.
...
**Clean execution reason code**:
The stable, path-free Foal-owned issue code used to explain a partial, skipped, or failed Clean TUI execution outcome. Category evidence wins over run-level fallback; when multiple category issues exist, operational failure outranks cancellation, which outranks an ordinary safety skip, while the first code at the same priority stays deterministic. The TUI maps only the code to short controlled copy and never forwards the source issue message or path. A cleaned outcome carries no reason, and run-level issues with no category are also shown once as path-free result notes.
_Avoid_: raw OS error, candidate path, category reason overwritten by run-level error, successful row with stale reason, TUI-only domain decision
...
The terminal Clean TUI surface that projects the shared final Result into path-free empty, cleaned, partial, skipped, failed, and canceled category outcomes plus stable path-free reason explanations for non-successful rows, deduplicated path-free notes for issues with no category, and Permanently deleted, Recycle Bin moved, and Affected byte totals. It never labels Affected bytes as released disk space. Item-level Result and history record the actual action and remain authoritative, including `context_canceled` skipped outcomes. The view ends the current preview and selection session; returning to the main menu discards that stale state, and entering Clean again starts a new eager preview scan.
