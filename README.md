...
- `clean`: conservative preview-first rules; mixed-action execution after explicit `--execute`. Permanent categories are the full 20-category matrix above (CLI `--allow-permanent`, TUI confirmation). The six Recycle Bin categories stay Recycle Bin.
