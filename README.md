...
Running `foal` (or the `fo` alias) with no arguments in an interactive terminal opens a read-only TUI: a main menu over the implemented commands, a clean preview browser, and read-only viewers for uninstall, status, and history. The Clean TUI shows default candidates and the same categorized skipped-by-default opportunities, including Chrome and Edge browser cache summaries when eligible, under its existing filters, keeps observed opportunity bytes distinct from `Potential space`, and supports cancellable reloads. Its category-selection preview starts empty; `tab` changes category focus, `space` toggles one category, `a` explicitly selects all, and `x` clears all. Every selection refreshes the shared Clean preview and remains preview-only: browsing and selection record no history sessions, write no detailed candidate list, and offer no confirmation or execution action. Scripts, pipes, and `--json` callers are unaffected and keep the deterministic help/command output.
