...
The TUI is a review and navigation surface over shared read models. Its Clean view displays the same `user_temp`, `crash_dumps`, `windows_error_reporting`, `explorer_thumbnail_cache`, `inet_cache`, `d3d_shader_cache`, `nvidia_dx_cache`, eligible Chrome/Edge `browser_cache`, and idle Application cache categories `vscode_cache` and `cursor_cache` Opportunity categories and permission-boundary notices without writing history or detailed lists. It does not duplicate deletion, uninstall, or path-safety logic.
