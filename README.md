...
- `clean`: conservative preview-first rules; permanent categories are `d3d_shader_cache`, `nvidia_dx_cache`, `browser_cache`, `vscode_cache`, and `cursor_cache` (CLI `--allow-permanent`, TUI confirmation authorization). Package/runtime permanent-matrix categories remain Recycle Bin until later tickets.
