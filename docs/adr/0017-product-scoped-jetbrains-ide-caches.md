...
- Permanent exclusions include Local History, file history, VCS Log, JCEF, plugins, logs, coverage, projects/data-source/editor/full-line/tmp/splash/metadata, unknown children, pre-2020 layouts, non-catalog products (Rider until its ticket; Fleet/Air/Gateway/Client; Android Studio; Toolbox/Installations/Daemon/Shared/dotPeek/ReSharper roots), regular files, and reparse points.
...
- Adding another compatible JetBrains IDE is a private catalog entry plus tests, not a new public category or pipeline (Rider/`resharper-host` remains #210).
