...
The Clean TUI gains category-level selection, explicit confirmation, executing, and result states over the shared Clean module. The shared Clean read model must expose category summaries and opt-in reclaimable bytes without turning paths into an execution manifest, and aggregate Recycle Bin capacity protection becomes a blocker for enabling the confirmed execution transition.
