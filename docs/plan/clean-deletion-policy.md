...
| `explorer_thumbnail_cache` | Opt-in | Proven (exact allowlist) | Yes | `delete_permanently` | Exact `thumbcache_*.db` and `iconcache_*.db` regular files under current-user `%LOCALAPPDATA%\Microsoft\Windows\Explorer` only; parent Explorer, ETL logs, `RecommendationsFilterList.json`, nested paths, and legacy `%LOCALAPPDATA%\IconCache.db` are never candidates. Missing matches ⇒ empty (no whole-root fallback). Locked DBs are skipped and the rest deleted (partial); Clean never stops Explorer. Evidence: `docs/research/explorer-thumbnail-and-inet-cache-allowlists.md`. |
| `inet_cache` | Opt-in | Proven (exact allowlist) | Yes | `delete_permanently` | Exact real directories `%LOCALAPPDATA%\Microsoft\Windows\INetCache\IE` and `...\INetCache\Low\IE` only; never whole INetCache, `Content.IE5`/`Low\Content.IE5` junctions, `Low` parent, `SuggestedSites.dat`, `Virtualized`, Office `Content.*`, or `thumbnails`. Missing both dirs ⇒ empty (no whole-root fallback). Locked files are skipped and the rest deleted (partial). Evidence: `docs/research/explorer-thumbnail-and-inet-cache-allowlists.md`. |
...
| `windows-temp` | Opt-in (exact-selection-only, machine-wide) | Not proven | No | `move_to_recycle_bin` | Stale direct children of the shared system temp directory `%SystemRoot%\Temp` (resolved from the `SystemRoot` env; blank/relative/UNC ⇒ silent absence). A direct child (file or directory) qualifies only when its deep Latest observed modification is at least 14 days old; unknown/future timestamps and incomplete inspection fail closed. The root is never a candidate and reparse points are never candidates or traversed. Requires a narrow, category-owned PathSafe carve-out for exactly the resolved `%SystemRoot%\Temp` subtree; the rest of the Windows tree stays rejected and Protection still applies inside. Exact-selection-only: excluded from `all`, group tokens, and TUI Select All; preview carries a path-free machine-wide impact notice. Non-elevated fail-closed: unreadable enumeration skips the whole category, per-item access denials are per-item skips, partial reclaim expected. Never elevation, ACL edits, service queries, or permanent deletion. Evidence: ADR 0030 / ADR 0032, `docs/plan/windows-temp-cleanup.md`. |
...
- The TUI starts with the 37 eligible rows described above selected when safely measurable (1 default + 36 permanent). Its one confirmation view separates Permanent deletion and Recycle Bin summaries, including category count, candidate count, measured bytes, per-category action, irreversible warning, and category-specific impact notices.
