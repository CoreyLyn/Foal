...
- `foal clean --dry-run --json` previews conservative cleanup candidates and reports exactly seven built-in skipped-by-default Opportunity categories: idle `user_temp` entries plus existence-observed `crash_dumps`, `windows_error_reporting`, `explorer_thumbnail_cache`, `inet_cache`, `d3d_shader_cache`, and `nvidia_dx_cache` current-user roots. Observed bytes are excluded from `Potential space`; the read-only Clean TUI presents the same review semantics without history or detailed-list writes. Browser caches remain excluded until running-application detection exists, the Recycle Bin is permanently excluded, developer-tool caches remain Review suggestions, and administrator-only caches remain permission-boundary notices. `foal clean --execute` does not run opportunity discovery, is the explicit confirmation path for fresh default candidates, and uses the Recycle Bin. Do not document permanent deletion or automatic elevation paths.
