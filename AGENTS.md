...
- `foal clean --dry-run --json` previews conservative cleanup candidates and reports skipped-by-default Opportunity categories: idle `user_temp` entries; existence-observed `crash_dumps`, `windows_error_reporting`, `explorer_thumbnail_cache`, `inet_cache`, `d3d_shader_cache`, and `nvidia_dx_cache` current-user roots; Chrome/Edge `browser_cache` when the browser is idle before and after complete profile cache inspection; and idle Application cache categories such as `vscode_cache` when Visual Studio Code is idle before and after exact allowlisted root inspection under the current user's standard Roaming AppData `Code` root. Observed bytes are excluded from `Potential space`; Clean TUI browsing and selection present the same review semantics without history or detailed-list writes. Confirmed TUI execution passes category identifiers only to shared Clean execution, which resolves fresh candidates, reports observation-only progress, and preserves normal result/history semantics; cancellation does not promise rollback. Browser review is gated by Chrome/Edge running-application detection: running browsers are reported as Running application skips and unknown process state is a recoverable diagnostic. Application cache review uses generic application-idle-before-and-after-inspection with the same fail-closed rules and never stops the application. The Recycle Bin is permanently excluded; developer-tool caches remain Review suggestions by default, or become Opt-in candidates when opted in via `--opt-in <dev-cache-name>` or `--opt-in dev-caches` or `--opt-in all`; Application cache categories such as `vscode_cache` are skipped by default and become Opt-in candidates via `--opt-in vscode_cache`, `--opt-in dev-caches`, `--opt-in all`, or Clean TUI selection; and administrator-only caches remain permission-boundary notices. `foal clean --execute` does not run opportunity discovery, browser/application running-application detection, or developer-tool cache probing by default, is the explicit confirmation path for fresh default candidates, and uses the Recycle Bin. When `--opt-in browser_cache` is used with `--execute`, browser running-application detection runs to skip active browsers; when `vscode_cache` or other Application cache categories are opted in, detection and exact allowlisted root inspection run fresh with idle-before-and-after gating; when dev caches are opted in, their paths are resolved only from environment variables and default paths (no tool probing), fresh validation is performed immediately before deletion, and aggregate per-volume Recycle Bin capacity pre-checks are applied before any deletion. Do not document permanent deletion or automatic elevation paths.
